...
(function( $, undefined ) {
	...
	$.mobile.loadPage = function( url, options ) {
		...
		var deferred = $.Deferred(),
                ...
                /**
                 * Ajouté par Eric Akakpo
                 * On veut indiquer au serveur s'il s'agit d'un dialog
                 * On injecte la donnée jqm_role=dialog dans les paramètres envoyé au serveuer
                 */
                //On ne change rien s'il s'agit d'un retour en arrière dans la navigation
                //Jquery mobile
                if(settings.reverse !== undefined && !settings.reverse){
                    //Si le paramètre existe déjà dans l'url on ne l'ajoute plus
                    if(typeof url === 'string' && url.search("jqm_role=dialog") < 0){
                        if(settings.role !== undefined && settings.role === "dialog"){
                            if(settings.data !== null && typeof settings.data === 'object')
                                settings.data.jqm_role = "dialog";
                            else
                            if(typeof settings.data === 'string' && settings.data.search("jqm_role=dialog") < 0)
                                settings.data += "&jqm_role=dialog";
                            else
                                settings.data = {jqm_role : "dialog"};
                        }
